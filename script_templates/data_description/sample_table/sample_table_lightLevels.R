...
table.sum <- ___  |>   #put the name of your wrangled data here
  ...
  #summarize by date & light levels 
  #(to get number of microhabs and area sampled per date)
  group_by(date, light.levels)  |>  
  summarize(
...
group_by(light.levels)  |>  

  #summarize by light levels
  summarize(
...
ft <- flextable(table.sum) |> 
  ...
  set_header_labels(year = "Light Levels",
                    dates = "Dates",
                    samples = "# of Samples",
